<template>
  <div class="nav-width-parent-container flex-row py-[37px] items-center w-full z-20">
    <img
        alt=""
        class="h-8 3xl:w-[194px] mr-[57px]" src="/imgs/xeron_logo.svg" @click="router.push('/')">
    <button
        @click="emits('toggleMenu')"
        class="flex lg:hidden cta bg-dark-blue text-white w-20 ml-auto">
      Menu
    </button>
    <div class="hidden lg:flex items-center w-full">
      <div ref="navRef" class="flex items-center gap-[16px] mr-auto">
        <router-link
          ...
          <div v-if="item.list" class="flex">
            ...
            <svg
                v-else
                aria-hidden="true"
                class="w-[10px] self-center mt-1 text-dark-blue" fill="none" stroke="currentColor" stroke-width="3.5"
      ...
      <div
          ref="btnDivRef"
          class="flex justify-end gap-[15px] ml-auto w-full relative">
        <button
            :class="[state.buttonDialogState === 'providers' ? 'bg-[#bcc6e0]' : 'bg-dark-blue']"
            class="cta text-white whitespace-nowrap flex items-center gap-1 justify-center">
            Pay a Bill
        </button>
        <button
            :class="[state.buttonDialogState === 'providers' ? 'bg-[#bcc6e0]' : 'bg-teal']"
            class="cta text-oil whitespace-nowrap flex items-center gap-1 justify-center"

            @click="state.buttonDialogState === 'patients'
          ...
          <svg
              v-else
              aria-hidden="true"
              class="w-[10px] self-center" fill="none" stroke="currentColor" stroke-width="3.5"
              viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <path d="M4.5 15.75l7.5-7.5 7.5 7.5" stroke-linecap="round" stroke-linejoin="round"></path>
          </svg>
        </button>
        <button
            :class="[state.buttonDialogState === 'patients' ? 'bg-[#bcc6e0]' : 'bg-dark-blue']"
            class="cta text-white   whitespace-nowrap flex items-center gap-1 justify-center"
            @click="state.buttonDialogState === 'providers'
             ? state.buttonDialogState= ''
